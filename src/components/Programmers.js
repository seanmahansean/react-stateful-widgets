...
export default function Programmers() {
  ...
  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em', 
    color: index ? "gold" : 'royalblue', // 🤔 color turns to gold, when celebrating
  };
      ...
      <div className='programmers'>
        {
          ...
          list.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => {setIndex(dev.id)}}>Feature</button>
            </div>
          )
        }
      </div>
      <div id='featured' style={style}>
        {
          ...
          index ? `🎉 Let's celebrate ${getNameOfFeatured()}! 🥳` : 'Pick an awesome programmer'
        }
      </div>
    </div>
  );
